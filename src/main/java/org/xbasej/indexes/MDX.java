...
public class MDX extends Index {
    ...
    public MDX(MDXFile ifile, DBF iDBF, short ipos) throws xBaseJException, IOException {
        ...
        while (reading > 0) {
            if (topNode == null) {
                ...
                MNode llNode = new MNode(mfile, key_per_Node, key_length, keyType, Index_record,
                        false);
    ...
    public MDX(String iname, String NDXString, DBF iDBF, MDXFile ifile, TagDescriptor inTagDesc,
            short pos, boolean unique) throws xBaseJException, IOException

    {
        ...
        if (!Character.isLetter(iname.charAt(0)))
            throw new xBaseJException(
                    "Invalid tag name " + iname + " first character not alphabetic");
    ...
    public int find_entry(NodeKey findKey, MNode aNode, int findrec)
        ...
        for (aNode.set_pos(0); aNode.get_pos() < until; aNode.pos_up()) {
            ...
            if ((findrec > 0)
                    && (rec == findrec)) /* found matching key and matching record number */
                return rec;
    ...
    private int update_entry(MNode aNode, NodeKey NDXkey, int recno, int oldrecno)
            throws IOException, xBaseJException {
        ...
        if (topNode == null || topNode.get_record_number() == 0) { /*
            ...
            if (topNode == null)
                topNode = new MNode(mfile, key_per_Node, key_length, keyType,
                        mfile.anchor.get_nextavailable(), false);
            workNode = topNode;
        ...
        if (aNode == null) { /* work to split the top Node */
            MNode newNode = new MNode(mfile, key_per_Node, key_length, keyType,
                    mfile.anchor.get_nextavailable(), true);
