...
public class DBF extends Object {
    ...
    private DBFTypes version = DBFTypes.DBASEIII;
    ...
    protected void openDBF(String DBFname) throws IOException, xBaseJException {
        ...
        if ((version != DBFTypes.DBASEIII) && (version != DBFTypes.DBASEIII_WITH_MEMO)
                && (version != DBFTypes.DBASEIV) && (version != DBFTypes.DBASEIV_WITH_MEMO)
                && (version != DBFTypes.FOXPRO_WITH_MEMO)) {
            String mismatch = Util.getxBaseJProperty("ignoreVersionMismatch").toLowerCase();
            if (mismatch != null
                    && (mismatch.compareTo("true") == 0 || mismatch.compareTo("yes") == 0))
        ...
        if (MDX_exist == 1) {
            try {
                ...
                if (missing != null
                        && (missing.compareTo("true") == 0 || missing.compareTo("yes") == 0))
                    MDX_exist = 0;
                else {
    ...
    protected void createDBF(String DBFname, DBFTypes format, boolean destroy)
        ...
        if (format != DBFTypes.DBASEIII && format != DBFTypes.DBASEIV
                && format != DBFTypes.DBASEIII_WITH_MEMO && format != DBFTypes.DBASEIV_WITH_MEMO
    ...
    public void addField(Field aField[]) throws xBaseJException, IOException {
        ...
        for (j = 1; j <= aField.length; j++) {
            ...
            if ((dbtobj == null) && ((aField[j - 1] instanceof MemoField)
                    || (aField[j - 1] instanceof PictureField)))
                newMemo = true;
            if (aField[j - 1] instanceof PictureField)
                setVersion(DBFTypes.FOXPRO_WITH_MEMO);
    ...
    /**
     * gets an Index object associated with the database. This index does not become the primary index. Written for the makeDBFBean application.
    ...
    /**
     * used to find a record with an equal string value. when done the record pointer and field contents will be changed only if the exact key is
     * found.
     * 
     * @param keyString a search string
     ...
     * 
     * @param lock - boolean lock record indicator
     * @throws xBaseJException org.xBaseJ Index not opened or not part of the database eof - end of file
     * @throws IOException Java error caused by called methods
     */
     ...
     * 
     * @param lock boolean lock record indicator
     * @throws xBaseJException org.xBaseJ Index not opened or not part of the database tof - top of file
     * @throws IOException Java error caused by called methods
     */
    ...
    /**
     * used to read the next record, after the current record pointer, in the database. when done the record pointer and field contents will be
     * changed.
     * 
     * @param lock - boolean lock record indicator
    ...
    /**
     * used to read the previous record, before the current record pointer, in the database. when done the record pointer and field contents will be
     * changed.
     * 
     * @param lock - boolean lock record indicator
    ...
    /**
     * used to position record pointer at the first record or index in the database. when done the record pointer will be changed. NO RECORD IS READ.
    ...
    /**
     * used to position record pointer at the last record or index in the database. when done the record pointer will be changed. NO RECORD IS READ.
     ...
     * 
     * @param lock - boolean lock indicator - locks the entire file during the write process and then unlocks the file.
     ...
     * 
     * @param lock - boolean lock indicator - locks the entire file during the write process and then unlocks the file. Also record lock will be
    ...
    protected void db_offset(DBFTypes format, boolean memoPresent) {
        ...
        if (format == DBFTypes.FOXPRO_WITH_MEMO)
            if (memoPresent)
                ...
                setVersion(DBFTypes.DBASEIV);
        else if (format == DBFTypes.DBASEIV_WITH_MEMO || format == DBFTypes.DBASEIV
                || MDX_exist == 1)
            if (memoPresent)
                setVersion(DBFTypes.DBASEIV_WITH_MEMO);
            else
                setVersion(DBFTypes.DBASEIV); // DBASEIV;
        else if (memoPresent)
            setVersion(DBFTypes.DBASEIII_WITH_MEMO);
        else
            setVersion(DBFTypes.DBASEIII);
    ...
    public void pack()
        ...
        File f = File.createTempFile("tempxbase", "tmp.dbf");
